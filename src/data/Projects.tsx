interface ProjectsType {
    title: string;
    description: string;
    image: string;
    GitHub: string;
}

export const Projects: ProjectsType[] = [
    {
        title: 'BetterMomsh',
        description: 'A Pregnancy mobile app with health monitor',
        image: 'https://imgur.com/fRcQxRY.png',
        GitHub: ' ',
    },
    {
        title: 'Portfolio',
        description: 'My personal portfolio website',
        image: 'https://imgur.com/iLzO2eX.png',
        GitHub: ' ',
    },
    {
        title: 'ChatterBox',
        description: 'A chat application that allows users to create chat rooms and chat with other users.',
        image: 'https://imgur.com/ptoo806.png',
        GitHub: ' ',
    },
    {
        title: 'GalleryGrid',
        description: 'Bootleg of Pinterest',
        image: 'https://imgur.com/sD6Ec0j.png',
        GitHub: '',
    },
    {
        title: 'Expense Tracker',
        description: 'A simple expense tracker that allows users to add and delete expenses.',
        image: 'https://imgur.com/X9gGcXj.png',
        GitHub: '',
    },

];